...
def build_sqrt_table (inputfilename, col_num=0) :
  return tuple(sqrt(float(i.split()[col_num])) for i in open(inputfilename).readlines()[1:])
...
def build_richter_plot (x, filename, point_type = "",linewidth="1", color = "black", label_data="label", min_bound = 50, max_bound = 0) :
	...
	if min_bound == 50 :
		...
		plt.plot(center, t, point_type, color=color, linewidth = linewidth, label=label_data + ';  $b=%10.2f$' % (-aCoeff))
...
def build_fractal_dimension_figure(x, y, graph_name, filename) :
 ...
 #   ax2.set_ylim(2.0, 4.0)
    ...
    ax2.set_ylabel(r"$D_c=\frac{\delta\ \log\left(C_{2}\left(r\right)\right)}{\delta\ \log\left(r\right)}$",fontsize=20)
    lns = lm+lr+df#+lres
    labs = [l.get_label() for l in lns]
    ax1.legend(lns, labs,  loc=4)
#	ax1.set_ylim(0.0000001, 0.1)
